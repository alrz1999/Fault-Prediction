...
from config import PREPROCESSED_DATA_SAVE_DIR, BOW_SAVE_MODEL_DIR, BOW_SAVE_PREDICTION_DIR
...
def train_model(train_release):
    train_df = train_release.import_line_level_df(PREPROCESSED_DATA_SAVE_DIR)
    ...
    clf_path = os.path.join(BOW_SAVE_MODEL_DIR, re.sub('-.*', '', train_release.release_name) + "-BoW-model.bin")
    pickle.dump(clf, open(clf_path, 'wb'))
    vectorizer_path = os.path.join(BOW_SAVE_MODEL_DIR, re.sub('-.*', '', train_release.release_name) + "-vectorizer.bin")
...
def predict_defective_files_in_releases(train_release, eval_releases):
    ...
    for rel in eval_releases:
        test_df = rel.import_line_level_df(PREPROCESSED_DATA_SAVE_DIR)
        ...
        result_df['train'] = [train_release.release_name] * len(Y_pred)
