...
from config import DATE_SAVE_DIR, SAVE_MODEL_DIR, SAVE_PREDICTION_DIR
...
def train_model(train_release):
    ...
    train_df = train_df[train_df['is_blank'] == False]
    train_df = train_df[train_df['is_test_file'] == False]
    ...
    clf_path = os.path.join(SAVE_MODEL_DIR, re.sub('-.*', '', train_release.release) + "-BoW-model.bin")
    pickle.dump(clf, open(clf_path, 'wb'))
    vectorizer_path = os.path.join(SAVE_MODEL_DIR, re.sub('-.*', '', train_release.release) + "-vectorizer.bin")
...
def predict_defective_files_in_releases(train_release, eval_releases):
    ...
    for rel in eval_releases:
        ...
        result_df.to_csv(os.path.join(SAVE_PREDICTION_DIR, rel.release + '.csv'), index=False)
